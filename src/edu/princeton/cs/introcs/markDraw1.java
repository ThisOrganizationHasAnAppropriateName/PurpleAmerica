...
/**
 * ****************************************************************************
 * Compilation: javac markDraw.java Execution: java markDraw Dependencies: none
 *
...
/**
 * The {@code markDraw} class provides a basic capability for creating drawings
 ...
 * <p>
 * <b>Getting started.</b>
 * To use standard drawing, you must have <tt>markDraw.class</tt> in your Java
 * classpath. If you used our autoinstaller, you should be all set. Otherwise,
 * download
 * <a href = "http://introcs.cs.princeton.edu/java/stdlib/markDraw.java">markDraw.java</a>
 * and put a copy in your working directory.
 * <p>
 * Now, type the following short program into your editor:
 * <pre>
 *   public class TestmarkDraw {
 *       public static void main(String[] args) {
 *           markDraw.setPenRadius(0.05);
 *           markDraw.setPenColor(markDraw.BLUE);
 *           markDraw.point(0.5, 0.5);
 *           markDraw.setPenColor(markDraw.MAGENTA);
 *           markDraw.line(0.2, 0.2, 0.8, 0.2);
 *       }
 *   }
 ...
 * {@code markDraw.line()} and {@code markDraw.point()} draw lines and points;
 * the methods {@code markDraw.setPenRadius()} and
 * {@code markDraw.setPenColor()} control the line thickness and color.
 * <p>
 ...
 * {@code markDraw.arc(0.0, 0.0, 1.0, 0, 90)} draws the arc of the unit circle
 ...
 * </ul>
 * <p>
 * For example, {@code markDraw.setPenRadius(0.025)} makes the thickness of the
 ...
 * example, {@code markDraw.setPenColor(markDraw.MAGENTA)} sets the pen color to
 * magenta.
 * <p>
 * <b>Canvas size.</b>
 ...
 * {@code markDraw.setCanvasSize(800, 800)} sets the canvas size to be
 * 800-by-800 pixels.
 * <p>
 ...
 * margin, you can call {@code markDraw.setScale(-.05, 1.05)}.
 * <p>
 ...
 *   markDraw.setFont(font);
 *   markDraw.text(0.5, 0.5, "Hello, World");
 * </pre>
 * <p>
 * <b>Images.</b>
 ...
 * {@code markDraw.clear(markDraw.LIGHT_GRAY)} clears the canvas to a shade of
 ...
 * results appear on the screen. {@code markDraw.show()} turns off animation
 * mode.
 * <p>
 ...
 * objects. That is, call {@code markDraw.show(0)} before and after the sequence
 ...
 * @author Kevin Wayne
 */
public final class markDraw1 implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static markDraw1 std = new markDraw1();
    ...
    public void drawByCounty(int year) throws Exception {
        ...
        markDraw1.setCanvasSize(Math.abs((int) ((xmax - xmin) * 12 + 20)), (int) ((ymax - ymin) * 15 + 20));

        markDraw1.setXscale(xmin - ((xmax - xmin) / 10), xmax + ((xmax - xmin) / 10));
        markDraw1.setYscale(ymin - ((ymax - ymin) / 10), ymax + ((ymax - ymin) / 10));

        markDraw1.setPenRadius(0);
        ...
        while (in.hasNext()) {
            ...
            try {
                if (state.compareTo("LA") == 0) {
                    markDraw1.setPenColor(colors[counter - 1].getMapColor(name.replace(" Parish", "")));
                } else {
                    markDraw1.setPenColor(colors[counter - 1].getMapColor(name));
                }
            } catch (Exception e) {
                try {
                    markDraw1.setPenColor(colors[counter - 1].getMapColor(name.replace(" city", "")));
                } catch (Exception x) {
                    markDraw1.setPenColor(0, 0, 0);
            ...
            //System.out.println(name);
            markDraw1.filledPolygon(polX, polY);
            frame.setTitle("Map US 2: " + year);
            in.nextLine();
        }
     ...
     * <tt>markDraw.BLACK</tt>, <tt>markDraw.BLUE</tt>, <tt>markDraw.CYAN</tt>,
     * <tt>markDraw.DARK_GRAY</tt>, <tt>markDraw.GRAY</tt>,
     * <tt>markDraw.GREEN</tt>,
     * <tt>markDraw.LIGHT_GRAY</tt>, <tt>markDraw.MAGENTA</tt>,
     * <tt>markDraw.ORANGE</tt>,
     * <tt>markDraw.PINK</tt>, <tt>markDraw.RED</tt>, <tt>markDraw.WHITE</tt>,
     * and
     * <tt>markDraw.YELLOW</tt>.
     *
     ...
     * <em>y</em><sub>1</sub>), ..., (<em>x</em><sub><em>n</em>&minus;1</sub>,
    ...
    private static Image getImage(String filename) {
        ...
        // in case file is inside a .jar (classpath relative to markDraw)
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = markDraw1.class.getResource(filename);
            if (url != null) {
                icon = new ImageIcon(url);
            }
        }
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(markDraw1.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            markDraw1.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = markDraw1.userX(e.getX());
            mouseY = markDraw1.userY(e.getY());
            mousePressed = true;
        }
    }
     ...
     */
    public static void main(String[] args) {
        markDraw1.square(.2, .8, .1);
        markDraw1.filledSquare(.8, .8, .2);
        markDraw1.circle(.8, .2, .2);

        markDraw1.setPenColor(markDraw1.BOOK_RED);
        markDraw1.setPenRadius(.02);
        markDraw1.arc(.8, .2, .1, 200, 45);

        // draw a blue diamond
        markDraw1.setPenRadius();
        markDraw1.setPenColor(markDraw1.BOOK_BLUE);
        double[] x = {.1, .2, .3, .2};
        double[] y = {.2, .3, .2, .1};
        markDraw1.filledPolygon(x, y);

        // text
        markDraw1.setPenColor(markDraw1.BLACK);
        markDraw1.text(0.2, 0.5, "black text");
        markDraw1.setPenColor(markDraw1.WHITE);
        markDraw1.text(0.8, 0.8, "white text");
    }
 ...
 *
 * Introduction to Programming in Java: An Interdisciplinary Approach by R.
 * Sedgewick and K. Wayne, Addison-Wesley, 2007. ISBN 0-321-49805-4.
 *
 * http://introcs.cs.princeton.edu
 *
 *
 * stdlib-package.jar is free software: you can redistribute it and/or modify it
 * under the terms of the GNU General Public License as published by the Free
 * Software Foundation, either version 3 of the License, or (at your option) any
 * later version.
 *
 * stdlib-package.jar is distributed in the hope that it will be useful, but
 * WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more
 * details.
 *
 * You should have received a copy of the GNU General Public License along with
 * stdlib-package.jar. If not, see http://www.gnu.org/licenses.
 * ***********************************************************************
 */
