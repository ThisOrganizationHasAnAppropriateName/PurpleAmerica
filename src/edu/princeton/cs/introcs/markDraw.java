 ...
 * @author Kevin Wayne
 */
public final class markDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static markDraw std = new markDraw();
    ...
    public void drawByCounty(int year) throws Exception {
        ...
        markDraw.setCanvasSize(Math.abs((int) ((xmax - xmin) * 12 + 20)), (int) ((ymax - ymin) * 15 + 20));

        markDraw.setXscale(xmin - ((xmax - xmin) / 10), xmax + ((xmax - xmin) / 10));
        markDraw.setYscale(ymin - ((ymax - ymin) / 10), ymax + ((ymax - ymin) / 10));

        markDraw.setPenRadius(0);
        ...
        while (in.hasNext()) {
            ...
            try {
                if (state.compareTo("LA") == 0) {
                    markDraw.setPenColor(colors[counter - 1].getMapColor(name.replace(" Parish", "")));
                } else {
                    markDraw.setPenColor(colors[counter - 1].getMapColor(name));
                }
            } catch (Exception e) {
                try {
                    markDraw.setPenColor(colors[counter - 1].getMapColor(name.replace(" city", "")));
                } catch (Exception x) {
                    markDraw.setPenColor(0, 0, 0);
            ...
            //System.out.println(name);
            markDraw.filledPolygon(polX, polY);
            markDraw.setFont();
            frame.setTitle("Map US 1: " + year);
            in.nextLine();
        }
    ...
    private static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = markDraw.class.getResource(filename);
            if (url != null) {
                icon = new ImageIcon(url);
            }
        }
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(markDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            markDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = markDraw.userX(e.getX());
            mouseY = markDraw.userY(e.getY());
            mousePressed = true;
        }
    }
     ...
     */
    public static void main(String[] args) {
        markDraw.square(.2, .8, .1);
        markDraw.filledSquare(.8, .8, .2);
        markDraw.circle(.8, .2, .2);

        markDraw.setPenColor(markDraw.BOOK_RED);
        markDraw.setPenRadius(.02);
        markDraw.arc(.8, .2, .1, 200, 45);

        // draw a blue diamond
        markDraw.setPenRadius();
        markDraw.setPenColor(markDraw.BOOK_BLUE);
        double[] x = {.1, .2, .3, .2};
        double[] y = {.2, .3, .2, .1};
        markDraw.filledPolygon(x, y);

        // text
        markDraw.setPenColor(markDraw.BLACK);
        markDraw.text(0.2, 0.5, "black text");
        markDraw.setPenColor(markDraw.WHITE);
        markDraw.text(0.8, 0.8, "white text");
    }
